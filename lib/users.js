...
exports.findShopById = function(id,cb){
    ShopBase.findOne({'id': id},'', function(err,user){
        if(user){
           return cb(null, user);
        }else{
            return cb(null,null);
...
exports.getShopPage = function(pageNumber, done){
    ...
    // find current page
    ShopBase.find().sort({weight:-1}).skip(pageNumber * 3).limit(3).exec(
...
exports.getUsersShops = function(username, done){
    ...
    this.findByUsername(username, function(err, user){
        ...
        var i = 0, len = user.shops.length, query = [];
        
        // build the query, using the users shop id's in user.shops
        while(i < len){
            
            query.push({"id": user.shops[i]});
            
            i++;
        }
        
        // get the shops
        ShopBase.find({"$or": query}, function(err, shops){
...
exports.buyShopItems = function(buyer, items, done){
    ...
    console.log(buyer + ' is buying shop items');
    //console.log(items);
    
    this.findByUsername(buyer, function(err, user){
    
        console.log('buyers wallet and storage.');
        ...
        items.forEach(function(item){
            ...
            //console.log(item);
            ShopBase.findOne({id: item.shopId}, '', function(err, shop){
            
                //console.log(shop.forSale);
                
                var i=0, len = shop.forSale.length;
              
                console.log('itemId: ' + item.itemId);
                while(i < len){
                    
                    console.log(shop.forSale[i]._id);
                    
                    if(String(shop.forSale[i]._id) === item.itemId){
                        
                        console.log('found item!');
                        //console.log(shop.forSale[i]);
                        
                        if(user.wallet >= shop.forSale[i].pebbleCost){
                            
                            console.log(shop.forSale.splice(i, 1));
