    ...
    this.getReserve(function (reserve) {
        ...
        Account.find({}, function (err, accounts) {
            ...
            while (i < len) {
                ...
                accountTotal += accounts[i].wallet;

                i += 1;

            }

            pebbleSum = reserve.wallet + accountTotal;

            // reset the clock
            reserve.sanity.lastCheck = String(now);
            ...
            reserve.sanity.history.push({

                worldTotal : reserve.worldTotal,
                pebbleSum : pebbleSum,
                accountTotal : accountTotal,
                reserveWallet : reserve.wallet,
                sain : Boolean(pebbleSum === reserve.worldTotal),
                checkDone : String(now),
                action : 'none'
            ...
            console.log('accounts total: ' + accountTotal);
            console.log('sanity: ' + Boolean(pebbleSum === reserve.worldTotal));
